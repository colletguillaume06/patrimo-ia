  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl text-text-primary">Carnet de contacts</h1>
          <p className="text-text-tertiary text-sm mt-1">{contacts.length} contact{contacts.length > 1 ? 's' : ''} · tous biens</p>
        </div>
      ...
      <div className="flex gap-3">
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
          className="h-10 px-3 rounded-xl bg-bg-secondary border border-border text-text-primary text-sm focus:outline-none">
          <option value="all" className="bg-bg-card">Tous les rôles</option>
          {Object.entries(ROLES).map(([k, v]) => <option key={k} value={k} className="bg-bg-card">{v.icon} {v.label}</option>)}
        </select>
      </div>

      {loading ? (
        <div className="grid grid-cols-2 gap-3">{[1,2,3,4].map(i => <div key={i} className="h-24 rounded-xl bg-bg-secondary/50 animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        <GlassCard className="py-16 text-center"><p className="text-text-tertiary">Aucun contact</p></GlassCard>
      ) : (
            ...
            return (
              <GlassCard key={c.id} hover className="p-4">
                <div className="flex items-start gap-3">
                  <div className="h-10 w-10 rounded-xl bg-bg-secondary flex items-center justify-center text-xl flex-shrink-0">{role.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-text-primary">{c.prenom} {c.nom}</p>
                    {c.entreprise && <p className="text-xs text-text-tertiary">{c.entreprise}</p>}
                    <p className="text-xs text-text-secondary">{role.label} · {c.property?.name ?? 'Tous biens'}</p>
                    ...
                    {c.notes && <p className="text-xs text-text-secondary mt-1 line-clamp-1">{c.notes}</p>}
                  </div>
                </div>
              </GlassCard>
            )
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-bg-card border border-border rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-text-primary">Nouveau contact</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="block text-xs text-text-tertiary mb-1">Bien associé</label>
                <select value={form.property_id} onChange={e => setForm(f => ({ ...f, property_id: e.target.value }))}
                  ...
                  <option value="" className="bg-bg-card">Tous les biens</option>
                  {properties.map(p => <option key={p.id} value={p.id} className="bg-bg-card">{p.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs text-text-tertiary mb-1">Rôle *</label>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2}
                  className="w-full px-3 py-2 rounded-lg bg-bg-secondary border border-border text-text-primary text-sm focus:outline-none resize-none" />
              </div>
