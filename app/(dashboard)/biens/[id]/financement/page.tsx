  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-display font-bold text-xl text-text-primary">Financement</h1>
      </div>

      {/* Formulaire prêt */}
      <GlassCard>
        <h2 className="font-display font-semibold text-text-primary mb-4">Paramètres du prêt</h2>
      ...
      {hasPret && kpis && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Mensualité totale', value: formatCurrency(kpis.mensualite_avec_assurance), sub: `dont ${formatCurrency(kpis.mensualite)} hors assurance`, color: 'text-text-primary' },
              ...
              { label: `Intérêts déductibles ${new Date().getFullYear()}`, value: formatCurrency(interetsAnnee), sub: 'à reporter en charges', color: 'text-success-text' },
          ...
          <GlassCard>
            <h3 className="font-display font-semibold text-text-primary mb-4">Évolution du capital restant dû</h3>
          ...
          <GlassCard>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-semibold text-text-primary">Tableau d'amortissement</h3>
              <div className="flex items-center gap-2 text-xs text-text-tertiary">
                <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="h-7 w-7 rounded-lg bg-bg-secondary flex items-center justify-center disabled:opacity-30 hover:bg-white/[0.10]"><ChevronLeft className="h-4 w-4" /></button>
                <span>{page + 1} / {totalPages}</span>
                <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="h-7 w-7 rounded-lg bg-bg-secondary flex items-center justify-center disabled:opacity-30 hover:bg-white/[0.10]"><ChevronDown className="h-4 w-4" /></button>
              </div>
            </div>
                  ...
                  {pageData.map(e => {
                    ...
                    return (
                      <tr key={e.numero} className={`border-b border-white/[0.04] text-right ${isCurrentMonth ? 'bg-blue-500/10' : 'hover:bg-white/[0.02]'}`}>
                        <td className="py-2.5 px-3 text-left text-text-secondary">{e.numero}</td>
                        <td className="py-2.5 px-3 text-text-tertiary whitespace-nowrap">{format(e.date, 'MM/yyyy')}</td>
                        <td className="py-2.5 px-3 text-text-primary font-medium">{formatCurrency(e.mensualite)}</td>
                        <td className="py-2.5 px-3 text-red-400">{formatCurrency(e.interets)}</td>
                        <td className="py-2.5 px-3 text-success-text">{formatCurrency(e.capital_amorti)}</td>
                        <td className="py-2.5 px-3 text-text-secondary">{formatCurrency(e.assurance)}</td>
