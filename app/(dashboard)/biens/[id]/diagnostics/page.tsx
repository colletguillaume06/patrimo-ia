    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={`/biens/${id}`} className="text-text-tertiary hover:text-text-primary text-sm flex items-center gap-1">
            <ChevronLeft className="h-4 w-4" /> {property?.name}
          </Link>
          <span className="text-text-secondary">/</span>
          <h1 className="font-display font-bold text-xl text-text-primary">Diagnostics obligatoires</h1>
        </div>
      ...
      <GlassCard>
        {loading ? (
          <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-12 rounded-lg bg-bg-secondary/50 animate-pulse" />)}</div>
        ) : (
                ...
                {TYPES.map(type => {
                  ...
                  if (!diag) return (
                    <tr key={type} className="border-b border-white/[0.04]">
                      <td className="py-3 px-3 text-text-tertiary">{DIAG_LABELS[type]}</td>
                  ...
                  return (
                    <tr key={type} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      <td className="py-3 px-3">
                        <p className="text-text-primary font-medium">{DIAG_LABELS[type]}</p>
                      ...
                      <td className="py-3 px-3 text-text-tertiary">{diag.resultat || '—'}</td>
                      <td className="py-3 px-3 text-text-tertiary whitespace-nowrap">{format(new Date(diag.date_realisation), 'dd/MM/yyyy')}</td>
                      ...
                      <td className="py-3 px-3 text-text-tertiary truncate max-w-[120px]">{diag.cabinet || '—'}</td>
                      <td className="py-3 px-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-bg-card border border-border rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-text-primary">Nouveau diagnostic</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as DiagType }))} className="w-full h-10 px-3 rounded-lg bg-bg-secondary border border-border text-text-primary text-sm focus:outline-none">
                  {TYPES.map(t => <option key={t} value={t} className="bg-bg-card">{DIAG_LABELS[t]}</option>)}
                </select>
              </div>
              {form.type === 'dpe' && (
                <div>
                  <label className="block text-xs text-text-tertiary mb-1">Classe énergétique</label>
                  <select value={form.valeur_dpe} onChange={e => setForm(f => ({ ...f, valeur_dpe: e.target.value as DPELettre }))} className="w-full h-10 px-3 rounded-lg bg-bg-secondary border border-border text-text-primary text-sm focus:outline-none">
                    <option value="" className="bg-bg-card">—</option>
                    {(['A','B','C','D','E','F','G'] as DPELettre[]).map(l => <option key={l} value={l} className="bg-bg-card">{l}</option>)}
                  </select>
                </div>
              )}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-tertiary mb-1">Date réalisation *</label>
                  <input type="date" value={form.date_realisation} onChange={e => setForm(f => ({ ...f, date_realisation: e.target.value }))} required className="w-full h-10 px-3 rounded-lg bg-bg-secondary border border-border text-text-primary text-sm focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs text-text-tertiary mb-1">Cabinet</label>
                  <input type="text" placeholder="Nom du cabinet" value={form.cabinet} onChange={e => setForm(f => ({ ...f, cabinet: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-bg-secondary border border-border text-text-primary placeholder-slate-600 text-sm focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-text-tertiary mb-1">Résultat</label>
                <input type="text" placeholder="Ex: négatif, classe C, 45 kWh/m²..." value={form.resultat} onChange={e => setForm(f => ({ ...f, resultat: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-bg-secondary border border-border text-text-primary placeholder-slate-600 text-sm focus:outline-none" />
