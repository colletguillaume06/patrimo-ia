    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={`/biens/${id}`} className="text-slate-400 hover:text-white text-sm flex items-center gap-1">
            <ChevronLeft className="h-4 w-4" /> {property?.name}
          </Link>
          <span className="text-slate-600">/</span>
          <h1 className="font-display font-bold text-xl text-white">Diagnostics obligatoires</h1>
        </div>
                  ...
                  return (
                    <tr key={type} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      <td className="py-3 px-3">
                        <p className="text-white font-medium">{DIAG_LABELS[type]}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#111E35] border border-white/[0.08] rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-white">Nouveau diagnostic</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as DiagType }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none">
              ...
              {form.type === 'dpe' && (
                <div>
                  ...
                  <select value={form.valeur_dpe} onChange={e => setForm(f => ({ ...f, valeur_dpe: e.target.value as DPELettre }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.date_realisation} onChange={e => setForm(f => ({ ...f, date_realisation: e.target.value }))} required className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Cabinet</label>
                  <input type="text" placeholder="Nom du cabinet" value={form.cabinet} onChange={e => setForm(f => ({ ...f, cabinet: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white placeholder-slate-600 text-sm focus:outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Résultat</label>
                <input type="text" placeholder="Ex: négatif, classe C, 45 kWh/m²..." value={form.resultat} onChange={e => setForm(f => ({ ...f, resultat: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white placeholder-slate-600 text-sm focus:outline-none" />
