  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/baux/${id}`} className="text-slate-400 hover:text-white text-sm flex items-center gap-1">
          <ChevronLeft className="h-4 w-4" /> {lease?.tenant_name}
        </Link>
        <span className="text-slate-600">/</span>
        <h1 className="font-display font-bold text-xl text-white">Régularisation des charges</h1>
      </div>

      <GlassCard>
        <h2 className="font-display font-semibold text-white mb-4">Type de charges</h2>
      ...
      {form.charges_type === 'provision' && (
        <GlassCard>
          <h2 className="font-display font-semibold text-white mb-4">Calcul de régularisation {form.annee}</h2>
          ...
          {charges_reelles > 0 && (
            <div className={`p-4 rounded-xl border space-y-2 mb-5 ${solde >= 0 ? 'border-green-400/20 bg-green-400/5' : 'border-red-400/20 bg-red-400/5'}`}>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-semibold">{formatCurrency(provisions)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Charges réelles {form.annee}</span>
                <span className="text-white font-semibold">{formatCurrency(charges_reelles)}</span>
              </div>
          ...
          <button onClick={handleCopy} disabled={charges_reelles === 0}
            className="flex items-center gap-2 h-9 px-5 rounded-lg bg-white/[0.06] border border-white/[0.10] text-slate-300 hover:text-white text-sm transition-all disabled:opacity-40">
