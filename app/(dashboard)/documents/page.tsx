  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl text-white">GED — Gestion documentaire</h1>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-40">
          ...
          <input type="text" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full h-10 pl-9 pr-4 rounded-xl bg-white/[0.06] border border-white/[0.10] text-white placeholder-slate-600 text-sm focus:outline-none" />
        </div>
        <select value={propFilter} onChange={e => setPropFilter(e.target.value)} className="h-10 px-3 rounded-xl bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none">
        ...
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)} className="h-10 px-3 rounded-xl bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#111E35] border border-white/[0.08] rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-white">Ajouter un document</h2>
            ...
            <form onSubmit={handleUpload} className="space-y-3">
              ...
              <div><label className="block text-xs text-slate-400 mb-1">Nom du document</label><input type="text" placeholder={file?.name ?? 'Titre du document'} value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none" /></div>
              <div><label className="block text-xs text-slate-400 mb-1">Catégorie *</label>
                <select value={form.categorie} onChange={e => setForm(f => ({ ...f, categorie: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none">
                  {Object.entries(CATEGORIES).map(([k, v]) => <option key={k} value={k} className="bg-[#111E35]">{v.icon} {v.label}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs text-slate-400 mb-1">Année fiscale</label>
                  <input type="number" placeholder="2026" value={form.annee_fiscale} onChange={e => setForm(f => ({ ...f, annee_fiscale: e.target.value }))} className="w-full h-10 px-3 rounded-lg bg-white/[0.06] border border-white/[0.10] text-white text-sm focus:outline-none" />
                </div>
              </div>
