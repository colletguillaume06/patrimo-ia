              ...
              <div className="mt-4 pt-4 border-t border-white/[0.06]">
                <form onSubmit={e => handleAdd(e, inc.id)} className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-sm font-medium text-[#0F172A] mb-1.5">Entreprise *</label><input type="text" value={form.entreprise} onChange={e => setForm(f => ({ ...f, entreprise: e.target.value }))} required className="w-full h-9 px-3 rounded-lg bg-bg-secondary border border-border text-[var(--text-primary)] text-sm focus:outline-none" /></div>
                    <div><label className="block text-sm font-medium text-[#0F172A] mb-1.5">Montant (€) *</label><input type="number" value={form.montant} onChange={e => setForm(f => ({ ...f, montant: e.target.value }))} required className="w-full h-9 px-3 rounded-lg bg-bg-secondary border border-border text-[var(--text-primary)] text-sm focus:outline-none" /></div>
                    <div><label className="block text-sm font-medium text-[#0F172A] mb-1.5">Date réception</label><input type="date" value={form.date_reception} onChange={e => setForm(f => ({ ...f, date_reception: e.target.value }))} className="w-full h-9 px-3 rounded-lg bg-bg-secondary border border-border text-[var(--text-primary)] text-sm focus:outline-none" /></div>
                    <div><label className="block text-sm font-medium text-[#0F172A] mb-1.5">Délai exécution (jours)</label><input type="number" value={form.delai_execution_jours} onChange={e => setForm(f => ({ ...f, delai_execution_jours: e.target.value }))} className="w-full h-9 px-3 rounded-lg bg-bg-secondary border border-border text-[var(--text-primary)] text-sm focus:outline-none" /></div>
                  </div>
