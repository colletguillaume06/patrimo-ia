...
import {
  ChevronLeft, Phone, Mail, Building2, FileText, Calendar,
  Euro, User, Shield, Edit3, Save, X, CheckCircle, AlertCircle, Clock
} from 'lucide-react'
...
export default function LocataireDetailPage() {
    ...
    const { data } = await supabase
      ...
      .select(`
        *,
        ...
        payments(id, status, amount, due_date, paid_date)
      `)
      .eq('id', id)
      .single()
    setLease(data)
  ...
  if (!lease) return <div className="text-center py-12" style={{ color: 'var(--text-secondary)' }}>Locataire introuvable</div>

  const payments = lease.payments ?? []
  const paid = payments.filter((p: any) => p.status === 'paid')
  const late = payments.filter((p: any) => p.status === 'late')
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {lease.is_active
            ...
            : <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-slate-100 text-slate-500">Parti</span>
          }
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={() => setEditing(false)} className="h-9 px-4 rounded-xl text-sm border"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Loyer mensuel', value: formatCurrency(lease.monthly_rent), color: 'var(--text-primary)' },
          ...
          { label: 'Dépôt de garantie', value: formatCurrency(lease.deposit ?? 0), color: '#1D4ED8' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <GlassCard>
          <h2 className="font-display font-semibold mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            <User className="h-4 w-4" /> Informations locataire
          </h2>
          <div className="space-y-3">
            {editing ? (
              <>
                {[
                  { key: 'tenant_name', label: 'Nom complet' },
                  { key: 'tenant_email', label: 'Email', type: 'email' },
                  { key: 'tenant_phone', label: 'Téléphone' },
                  { key: 'tenant_address_before', label: 'Adresse précédente' },
              ...
              <div className="space-y-2">
                {[
                  { icon: User, label: lease.tenant_name },
                  { icon: Mail, label: lease.tenant_email || '—' },
                  { icon: Phone, label: lease.tenant_phone || '—' },
                  { icon: Building2, label: lease.property?.name || '—' },
          ...
          {editing ? (
            <div className="space-y-3">
              {[
                ...
                { key: 'guarantor_phone', label: 'Téléphone garant' },
                { key: 'guarantor_email', label: 'Email garant', type: 'email' },
              ].map(({ key, label, type = 'text' }) => (
                <div key={key}>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>{label}</label>
                  <input type={type} value={form[key]} onChange={e => setForm((f: any) => ({ ...f, [key]: e.target.value }))}
            ...
            <div className="space-y-2">
              {lease.guarantor_name ? (
                <>
                  <div className="flex items-center gap-3 py-2 px-3 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                    <User className="h-4 w-4" style={{ color: 'var(--text-tertiary)' }} />
                    <span className="text-sm" style={{ color: 'var(--text-primary)' }}>{lease.guarantor_name}</span>
                  </div>
                  {lease.guarantor_phone && (
                    <div className="flex items-center gap-3 py-2 px-3 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                      <Phone className="h-4 w-4" style={{ color: 'var(--text-tertiary)' }} />
                      <span className="text-sm" style={{ color: 'var(--text-primary)' }}>{lease.guarantor_phone}</span>
                    </div>
                  )}
                </>
              ) : (
        ...
        <GlassCard>
          <h2 className="font-display font-semibold mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            <FileText className="h-4 w-4" /> Bail
          </h2>
          {editing ? (
            <div className="space-y-3">
              {[
                { key: 'monthly_rent', label: 'Loyer HC (€)', type: 'number' },
                { key: 'monthly_charges', label: 'Charges (€)', type: 'number' },
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={form.is_active} onChange={e => setForm((f: any) => ({ ...f, is_active: e.target.checked }))} className="h-4 w-4" />
                <label className="text-sm" style={{ color: 'var(--text-secondary)' }}>Bail actif</label>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              {[
                { label: 'Loyer HC', value: formatCurrency(lease.monthly_rent) },
                { label: 'Charges', value: formatCurrency(lease.monthly_charges ?? 0) },
                { label: 'Total', value: formatCurrency((lease.monthly_rent ?? 0) + (lease.monthly_charges ?? 0)) },
                { label: 'Dépôt', value: formatCurrency(lease.deposit ?? 0) },
                { label: 'Début', value: lease.start_date ? format(new Date(lease.start_date), 'd MMMM yyyy', { locale: fr }) : '—' },
                { label: 'Fin', value: lease.end_date ? format(new Date(lease.end_date), 'd MMMM yyyy', { locale: fr }) : 'Indéterminée' },
              ].map(({ label, value }) => (
                <div key={label} className="flex justify-between items-center py-1.5 px-3 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                  <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{label}</span>
                  <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{value}</span>
                </div>
              ))}
              <Link href={`/baux`} className="flex items-center justify-center gap-2 mt-2 h-8 rounded-lg text-xs font-medium border transition-colors hover:opacity-80"
                style={{ borderColor: '#1D4ED8', color: '#1D4ED8', background: '#EFF6FF' }}>
                <FileText className="h-3.5 w-3.5" /> Voir le bail complet
              </Link>
            </div>
          )}
        </GlassCard>
        ...
        <GlassCard>
          <h2 className="font-display font-semibold mb-4 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
            <Euro className="h-4 w-4" /> Historique des paiements
          </h2>
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {payments.length === 0 ? (
              <p className="text-sm italic" style={{ color: 'var(--text-tertiary)' }}>Aucun paiement enregistré</p>
              ...
              return (
                <div key={p.id} className="flex items-center justify-between py-1.5 px-3 rounded-lg"
                  ...
                  <div className="flex items-center gap-2">
                    <Icon className="h-3.5 w-3.5 flex-shrink-0" style={{ color: colors[p.status] }} />
                    <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                      {format(new Date(p.due_date), 'MMM yyyy', { locale: fr })}
                    </span>
                  </div>
                  <span className="text-sm font-medium font-mono" style={{ color: colors[p.status] }}>
      ...
      <GlassCard>
        ...
        {editing ? (
          ...
          <p className="text-sm" style={{ color: lease.notes ? 'var(--text-primary)' : 'var(--text-tertiary)' }}>
            {lease.notes || 'Aucune note'}
          </p>
        )}
      </GlassCard>
    </div>
