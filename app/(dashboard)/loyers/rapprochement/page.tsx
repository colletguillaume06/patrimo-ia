  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/loyers" className="text-slate-400 hover:text-white text-sm flex items-center gap-1"><ChevronLeft className="h-4 w-4" /> Loyers</Link>
        <span className="text-slate-600">/</span>
        <h1 className="font-display font-bold text-xl text-white">Rapprochement bancaire</h1>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-display font-semibold text-white capitalize min-w-36 text-center">{moisLabel}</span>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Attendu', value: formatCurrency(total_attendu), color: 'text-white' },
      ...
      <GlassCard>
        <h2 className="font-display font-semibold text-white mb-4">Rapprochement — {moisLabel}</h2>
        {loading ? (
            ...
            <table className="w-full text-sm">
                ...
                {payments.map(p => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="py-3 px-3 text-white font-medium">{p.lease?.tenant_name ?? '—'}</td>
                      <td className="py-3 px-3 text-white">{formatCurrency(attendu)}</td>
              ...
              <tfoot>
                <tr className="border-t border-white/[0.10]">
                  <td colSpan={2} className="py-2.5 px-3 text-xs font-semibold text-white">TOTAL</td>
                  <td className="py-2.5 px-3 text-white font-bold">{formatCurrency(total_attendu)}</td>
