...
export function ProfileCommerce({ property: p }: ProfileCommerceProps) {
  ...
  return (
    <div className="space-y-6">
      <GlassCard glow="cyan" className="p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            {p.bail_type && <p className="text-xs text-text-secondary mt-0.5">Bail {p.bail_type}</p>}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Loyer HT mensuel', value: formatCurrency(loyer_ht), color: 'text-text-primary' },
            { label: 'Révision dans', value: monthsToEnd !== null ? `${monthsToEnd} mois` : '—', color: isPreavisAlert ? 'text-red-400' : 'text-amber-400' },
            { label: 'Nouveau loyer estimé', value: newRent ? formatCurrency(newRent) : '—', color: 'text-success-text' },
      ...
      {startDate && (
        <GlassCard>
          <h3 className="font-display font-semibold text-text-primary mb-4">Timeline bail 3-6-9</h3>
            ...
            {[3, 6, 9].map(year => {
              ...
              return (
                <div key={year} className="flex items-center gap-4 mb-4 pl-10 relative">
                  ...
                  <div>
                    <p className="text-sm font-medium text-text-primary">Fin période {year} ans</p>
                    <p className="text-xs text-text-secondary">{date.toLocaleDateString('fr-FR')} · {isPast ? 'Passé' : `Dans ${monthsUntil(date.toISOString())} mois`}</p>
                  </div>
                </div>
              )
            })}
      ...
      {/* Checklist */}
      <GlassCard>
        <h3 className="font-display font-semibold text-text-primary mb-4">Obligations bail commercial</h3>
        <div className="space-y-2">
          {checklistItems.map(({ label, done }) => (
            <div key={label} className="flex items-center gap-3 p-2.5 rounded-lg">
              <CheckCircle2 className={`h-4 w-4 flex-shrink-0 ${done ? 'text-success-text' : 'text-slate-700'}`} />
              <span className={`text-sm ${done ? 'text-text-secondary' : 'text-text-secondary'}`}>{label}</span>
            </div>
          ))}
        </div>
      </GlassCard>
