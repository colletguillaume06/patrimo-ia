    ...
    <div className="space-y-6">
      <GlassCard glow="amber" className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'RevPAR', value: formatCurrency(revPar), color: 'text-text-primary' },
            { label: "Taux d'occupation", value: formatPct(occupancyRate), color: 'text-amber-400' },
            { label: 'Recettes BIC', value: formatCurrency(totalRevenue), color: 'text-success-text' },
            { label: 'Nuits réservées', value: `${totalNights}`, color: totalNights > 100 ? 'text-red-400' : 'text-text-primary' },
      ...
      <GlassCard>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-display font-semibold text-text-primary">Compteur annuel (loi ELAN)</h3>
          {isNearLimit && (
        ...
        <div className="flex items-center justify-between mb-2 text-sm">
          <span className="text-text-tertiary">{totalNights} nuits utilisées</span>
          <span className="font-semibold text-text-primary">{nightsRemaining} restantes</span>
        </div>
        ...
        <div className="flex justify-between text-xs text-text-secondary mt-1">
          <span>0 nuits</span>
          <span className={pctUsed > 83 ? 'text-red-400' : 'text-text-tertiary'}>{Math.round(pctUsed)}%</span>
          <span>120 nuits</span>
        </div>
        {isNearLimit && (
      ...
      <GlassCard>
        <h3 className="font-display font-semibold text-text-primary mb-4">Réservations récentes</h3>
        {bookings.length === 0 ? (
          <p className="text-sm text-text-secondary">Aucune réservation enregistrée cette année</p>
        ) : (
          <div className="space-y-2">
            {bookings.slice(0, 8).map(b => (
              <div key={b.id} className="flex items-center justify-between py-2 border-b border-border">
                <div>
                  <p className="text-sm text-text-secondary">{b.guest_name ?? 'Voyageur anonyme'}</p>
                  <p className="text-xs text-text-secondary">{b.check_in} → {b.check_out} ({b.nights} nuits)</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-success-text">{formatCurrency(b.total_revenue ?? b.nightly_rate * (b.nights ?? 1))}</p>
                  <p className="text-xs text-text-secondary">{formatCurrency(b.nightly_rate)}/nuit</p>
                </div>
              </div>
            ))}
          </div>
