    ...
    <div className="space-y-6">
      <GlassCard glow="blue" className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Revenus bruts', value: formatCurrency(revenus_bruts), color: 'text-text-primary' },
      ...
      <GlassCard>
        <h3 className="font-display font-semibold text-text-primary mb-4">Charges déductibles détectées</h3>
        {deductibleExpenses.length === 0 ? (
          <p className="text-sm text-text-secondary">Aucune charge enregistrée pour {new Date().getFullYear()}</p>
        ) : (
          <div className="space-y-2">
            {deductibleExpenses.map(e => (
              <div key={e.id} className="flex items-center justify-between py-2 border-b border-border">
                <div>
                  <span className="text-sm text-text-secondary">{e.description ?? e.category}</span>
                  <span className="ml-2 text-xs text-text-secondary capitalize">{e.category}</span>
                </div>
                <span className="text-sm font-semibold text-red-400">-{formatCurrency(e.amount)}</span>
              </div>
            ))}
            <div className="flex items-center justify-between pt-2">
              <span className="text-sm font-semibold text-text-primary">Total déductible</span>
