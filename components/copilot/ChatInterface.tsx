...
export function ChatInterface({ initialMessages = [], propertyId, initialQuestion }: ChatInterfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-bg-primary rounded-2xl border border-border overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
        ...
        <div>
          <p className="font-display font-semibold text-text-primary">Propilot Copilot</p>
          <p className="text-xs text-text-secondary">Expert en immobilier et fiscalité française</p>
        </div>
        <div className="ml-auto flex items-center gap-1.5">
          <span className={`h-2 w-2 rounded-full ${apiError ? 'bg-red-400' : 'bg-green-400 animate-pulse'}`} />
          <span className={`text-xs ${apiError ? 'text-red-400' : 'text-success-text'}`}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            ...
            <h3 className="font-display font-semibold text-text-primary text-lg mb-2">Votre copilote immobilier</h3>
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-end gap-3 p-3 rounded-xl bg-bg-tertiary/30 border border-border focus-within:border-blue-500/30 transition-all">
          <textarea
            value={input}
            ...
            rows={1}
            className="flex-1 bg-transparent text-sm text-text-primary placeholder-slate-600 focus:outline-none resize-none max-h-32"
          ...
          >
            <Send className="h-3.5 w-3.5 text-text-primary" />
          </button>
        </div>
