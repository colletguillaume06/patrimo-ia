    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-bg-card border border-border rounded-2xl p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-display font-semibold text-text-primary">Envoyer une relance</h2>
        ...
        <div className="space-y-4 mb-6">
          <div className="p-4 rounded-xl bg-bg-secondary/50 border border-border">
            ...
            <p className="text-sm font-medium text-text-primary">{payment.lease?.tenant_name ?? 'N/A'}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl bg-bg-secondary/50 border border-border">
              <p className="text-xs text-text-secondary mb-0.5">Montant dû</p>
              <p className="text-sm font-semibold text-text-primary">{formatCurrency(payment.amount)}</p>
            </div>
            <div className="p-3 rounded-xl bg-bg-secondary/50 border border-border">
              <p className="text-xs text-text-secondary mb-0.5">Relance n°</p>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 h-10 rounded-lg border border-border text-text-tertiary hover:text-text-primary text-sm transition-all">
            Annuler
          </button>
          <button
            onClick={handleSend}
