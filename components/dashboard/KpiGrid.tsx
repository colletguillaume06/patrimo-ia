...
export function KpiGrid({ metrics }: KpiGridProps) {
  const kpis = [
    ...
    },
    {
      ...
      trendColor: metrics.rendement_moyen >= 4 ? 'text-[#0E7A4F]' : 'text-[#9B9589]',
    },
  ]
      ...
      {kpis.map((kpi) => (
        <GlassCard key={kpi.label} hover glow={kpi.glow} className="p-5 pt-6">
          <div className="flex items-start justify-between mb-3">
            ...
            <p className="text-[12px] font-medium text-[#9B9589] uppercase tracking-wide">
              {kpi.label}
            </p>
          ...
          <p className="text-[26px] font-semibold text-[#1A1714] tracking-tight mb-2"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[12px] text-[#9B9589]">{kpi.sub}</span>
          </div>
        </GlassCard>
      ))}
    </div>
