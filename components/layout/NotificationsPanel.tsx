...
const severityConfig = {
  high: { color: 'text-red-400', bg: 'bg-red-400/10', dot: 'bg-red-500' },
  medium: { color: 'text-amber-400', bg: 'bg-amber-400/10', dot: 'bg-amber-500' },
  low: { color: 'text-blue-400', bg: 'bg-blue-400/10', dot: 'bg-blue-500' },
}
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="relative h-9 w-9 rounded-lg bg-white/[0.04] border border-white/[0.08] flex items-center justify-center hover:bg-white/[0.07] transition-colors"
      >
      ...
      {open && (
        <div className="absolute right-0 top-11 w-80 bg-[#111E35] border border-white/[0.08] rounded-2xl shadow-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
            <h3 className="font-display font-semibold text-white text-sm">
              Alertes {count > 0 && <span className="text-slate-500">({count})</span>}
            </h3>
            <button onClick={() => setOpen(false)} className="h-6 w-6 rounded-md flex items-center justify-center hover:bg-white/[0.06]">
          ...
          <div className="max-h-96 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center px-4">
                ...
                <p className="text-xs text-slate-500 mt-1">Aucune alerte en cours</p>
              </div>
            ) : (
